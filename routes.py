...
def booking():
    ...
    if request.method == 'POST':
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            try:
                ...
                if appointment_time_pst.hour < 8 or appointment_time_pst.hour >= 17:
                    ...
                    return jsonify({
                        'error': 'Please select a time between 8 AM and 5 PM PST'
                    }), 400
                ...
                try:
                    ...
                    db.session.rollback()
                    return jsonify({
                        'error': 'An error occurred while saving your appointment. Please try again.'
                    }), 500
                ...
                if success:
                    ...
                    return jsonify({
                        'success': True,
                        'redirect': url_for('index'),
                        'message': f'Appointment booked successfully! However, there was an issue sending the confirmation email: {error_message}'
                    })
                    
            except ValueError as e:
                app.logger.error(f"Invalid date format in appointment submission: {str(e)}")
                return jsonify({
                    'error': 'Invalid date format. Please select a valid time slot.'
                }), 400
        ...
        # Handle regular form submission (fallback)
            ...
            if form.validate_on_submit():
                try:
                    # ... existing regular form submission logic ...
                    return redirect(url_for('index'))
                except Exception as e:
                    app.logger.error(f"Error in regular form submission: {str(e)}")
